...
def make_chart(df: pd.DataFrame, name: str):
    d = df.iloc[-260:]  # 최근 1년 남짓
    fig = make_subplots(
        rows=3, cols=1, shared_xaxes=True, vertical_spacing=0.04,
        row_heights=[0.6, 0.2, 0.2],
        subplot_titles=(f"{name} 일봉 + 이동평균", "RSI(14)", "MACD"),
    )
    fig.add_trace(
        go.Candlestick(
            x=d.index, open=d["Open"], high=d["High"], low=d["Low"], close=d["Close"],
            increasing_line_color="red", decreasing_line_color="blue", name="일봉",
        ),
        row=1, col=1,
    )
    colors = {"MA5": "#999", "MA20": "#f0a", "MA60": "#0a0",
              "MA120": "#fa0", "MA240": "#00f"}
    for ma, c in colors.items():
        fig.add_trace(
            go.Scatter(x=d.index, y=d[ma], line=dict(color=c, width=1.2), name=ma),
            row=1, col=1,
        )
    fig.add_trace(
        go.Scatter(x=d.index, y=d["RSI"], line=dict(color="#a0f", width=1), name="RSI"),
        row=2, col=1,
    )
    fig.add_hline(y=70, line_dash="dot", line_color="red", row=2, col=1)
    fig.add_hline(y=30, line_dash="dot", line_color="green", row=2, col=1)
    fig.add_trace(
        go.Bar(x=d.index, y=d["MACD_hist"], name="MACD Hist",
               marker_color=np.where(d["MACD_hist"] >= 0, "red", "blue")),
        row=3, col=1,
    )
    fig.update_layout(
...
with st.sidebar:
    ...
    if is_kr:
        code = st.text_input("종목코드 (6자리)", value="005930",
                             help="예: 삼성전자 005930, SK하이닉스 000660")
    else:
        code = st.text_input("티커", value="AAPL", help="예: AAPL, MSFT, NVDA, TSLA")
    years = st.slider("데이터 기간(년)", 2, 5, 3, help="240일선 계산을 위해 최소 2년 필요")
    run = st.button("분석 실행", type="primary", use_container_width=True)

tick = krx_tick_round if True else None  # placeholder
...
if run:
    ...
    try:
        ...
        if raw is None or len(raw) < 260:
            st.error("데이터가 부족함 (240일선 계산에 최소 1년 이상 필요). 입력을 확인할 것.")
            st.stop()
        ...
        st.subheader(f"{icon} 종합 신호: **{verdict}**  (점수 {total:+d})")
        ...
        st.info(
            f"매수 추정 근거: **{est['buy1'][0]}** 지지선 / "
            f"매도 추정 근거: **{est['sell1'][0]}** 저항선 / "
            f"피벗 기준가 {money(est['pivot'])}, ATR {money(est['atr'])}"
        )
        ...
        st.warning("⚠️ 본 프로그램의 신호와 추정가는 과거 데이터 기반 기술적 참고 지표일 뿐, "
                   "투자 권유가 아님. 투자 판단과 손실 책임은 이용자 본인에게 있음.")
    except Exception as e:
        st.error(f"오류 발생: {e}")
else:
    st.markdown(
        "왼쪽 사이드바(모바일은 좌측 상단 `>` 버튼)에서 **시장과 종목**을 선택하고 "
        "`분석 실행`을 누르면\n"
        "1. 일봉 데이터를 자동 수집하고\n"
        "2. 6개 지표를 점수화해 매수/관망/매도 신호를 내고\n"
        "3. 지지/저항 레벨에서 **다음날 매수·매도 추정가**를 산출함."
    )
